...
service / on new http:Listener(8080) {
    ...
    resource function post users(@http:Payload RegisterRequest newUser) returns UserResponse|ErrorResponse {
        sql:ParameterizedQuery checkQuery = `SELECT ID FROM login WHERE Email = ${newUser.Email}`;
        record {| int ID?; |}|sql:Error emailCheck = dbClient->queryRow(checkQuery);

        if emailCheck is record {| int ID?; |} {
            if emailCheck.ID is int {
                return {
